...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        r"""
        Log reward, obs and action to Tensorboard on step.

        Returns
        -------
        True
        """
        if len(self.locals["rewards"]) == 1:  # Single environment
            self.logger.record("reward", self.locals["rewards"][0])

            if "obs" in self.locals["infos"][0]:
                for key, value in self.locals["infos"][0]["obs"].items():
                    self.logger.record(f"obs/{key}", value)

            if "action" in self.locals["infos"][0]:
                for key, value in self.locals["infos"][0]["action"].items():
                    self.logger.record(f"actions/{key}", value)
        else:  # Multiple environments
            # Number of digits to pad the index
            digits = ceil(log10(len(self.locals["rewards"]) + 1))

            for i, _ in enumerate(self.locals["rewards"]):
                i_str = '{num:0{width}d}'.format(num=i, width=digits)

                self.logger.record(f"reward/{i_str}", self.locals["rewards"][i])

                if "obs" in self.locals["infos"][i]:
                    for key, value in self.locals["infos"][i]["obs"].items():
                        self.logger.record(f"delays/{i_str}/{key}", value)

                if "action" in self.locals["infos"][i]:
                    for key, value in self.locals["infos"][i]["action"].items():
                        self.logger.record(f"parameters/{i_str}/{key}", value)
